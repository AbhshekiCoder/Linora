...
const FooterBanner = () => {
  ...
  return (
    <div className="w-full flex max-md:flex-wrap justify-between mt-10 relative px-4 pb-8 bg-white">
        <div className='min-h-full w-full flex justify-center items-center p-6'>
           ...
           <p className='text-lg font-medium'>Discover more from Linora Home</p>
           </div>
        </div>

      <div className="max-w-5xl m-auto max-md:w-full">
        ...
        <button
          onClick={() => scroll(-400)}
          className="absolute  top-1/2 transform -translate-y-1/2 bg-white rounded-full p-3 shadow-lg z-10 hover:bg-gray-100 transition-all  max-sm:hidden"
        ...
        <button
          onClick={() => scroll(400)}
          className="absolute right-24 top-1/2 transform -translate-y-1/2 bg-white rounded-full max-sm:hidden p-3  shadow-lg z-10   hover:bg-gray-100 transition-all"
          style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
        >
        ...
        >
          {categories.map((category, index) => (
            <div 
              key={index} 
              className="min-w-[250px] h-[600px] flex flex-col group cursor-pointer transition-all duration-300 hover:scale-[1.02]"
            >
              <div className="overflow-hidden rounded-md mb-3 h-[600px]">
                <img 
